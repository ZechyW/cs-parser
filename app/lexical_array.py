...
class LexicalArray:
    ...
    def enumerate_input(user_input):
        ...
        """
        # Start by tokenising
        tokens = LexicalArray.tokenise(user_input)

        # Look up the tokens in the available lexica
        mapped_tokens = LexicalArray.lookup_tokens(tokens)
    ...
    def lookup_tokens(tokens, mapped=None):
        ...
        for length in reversed(range(len(tokens))):
            ...
            if len(possible_entries) == 0:
                ...
                return LexicalArray.lookup_tokens(tokens[length + 1:], mapped)
